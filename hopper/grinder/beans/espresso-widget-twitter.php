...
This is a clone of the WordPress.com JetPack twitter widget with added functionality to link the Title to twitter or not.
orginal: http://jetpack.me
...
class Espresso_Twitter_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( empty( $title ) ) $title = __( 'Twitter Updates', 'espresso' );
		...
		if ( 'error' != $tweets ) :
			...
			if ( 401 == get_transient( 'widget-twitter-response-code-' . $this->number ) )
				echo '<p>' . wp_kses( sprintf( __( 'Error: Please make sure the Twitter account is <a href="%s">public</a>.', 'espresso' ), 'http://support.twitter.com/forums/10711/entries/14016' ), array( 'a' => array( 'href' => true ) ) ) . '</p>';
			else
				echo '<p>' . esc_html__( 'Error: Twitter did not respond. Please wait a few minutes and refresh this page.', 'espresso' ) . '</p>';
		endif;
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['linktitle'] = isset( $new_instance['linktitle'] );
	...
	function form( $instance ) {
		//Defaults
		$instance = wp_parse_args( (array) $instance, array( 'account' => '', 'title' => '', 'show' => 5, 'hidereplies' => false ,'includeretweets'=>false,'hidereplies'=>false,'beforetimesince'=>'','linktitle'=>false) );
		...
		<p><label for="' . $this->get_field_id( 'account' ) . '">' . esc_html__( 'Twitter username:', 'espresso' ) . ' <a href="http://support.wordpress.com/widgets/twitter-widget/#twitter-username" target="_blank">( ? )</a>
		<input class="widefat" id="' . $this->get_field_id( 'account' ) . '" name="' . $this->get_field_name( 'account' ) . '" type="text" value="' . $account . '" />
		</label></p>';
		echo '<p><label for="' . $this->get_field_id( 'linktitle' ) . '"><input id="' . $this->get_field_id( 'linktitle' ) . '" class="checkbox" type="checkbox" name="' . $this->get_field_name( 'linktitle' ) . '"';
		if ( $link_title )
		...
		if ( $hidereplies )
			echo ' checked="checked"';
		echo ' /> ' . esc_html__( 'Hide replies', 'espresso' ) . '</label></p>';
		...
		if ( $include_retweets )
			echo ' checked="checked"';
		echo ' /> ' . esc_html__( 'Include retweets', 'espresso' ) . '</label></p>';

		echo '<p><label for="' . $this->get_field_id( 'beforetimesince' ) . '">' . esc_html__( 'Text to display between tweet and timestamp:', 'espresso' ) . '
...
add_action( 'widgets_init', 'espresso_twitter_widget_init' );
function espresso_twitter_widget_init() {
	register_widget( 'Espresso_Twitter_Widget' );
}
